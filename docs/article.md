...
> [!TIP]
> Для глибшого концептуального розуміння бекграунду та архітектурного бачення проєкту, див. презентацію `docs/Hermes_Agent_Evolution.pdf`. Вона доповнює цю практичну інструкцію.
...
- `README.md`
- `deploy/hermes-lxc.env.example`
- `deploy/hermes.env.example`
- `deploy/authorized_keys.example`
- `deploy/port-forwards.tsv.example`
- `deploy/npm-proxy-hosts.tsv.example`
