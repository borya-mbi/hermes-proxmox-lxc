...
validate_port "$HERMES_PUBLIC_PORT" || die "Invalid HERMES_PUBLIC_PORT: $HERMES_PUBLIC_PORT"
validate_port "$HERMES_ADMIN_PORT" || die "Invalid HERMES_ADMIN_PORT: $HERMES_ADMIN_PORT"
...
api_key_value="$(awk -F= '/^OPENAI_API_KEY=/{print $2}' "$APP_ENV_FILE" | head -n1 || true)"
